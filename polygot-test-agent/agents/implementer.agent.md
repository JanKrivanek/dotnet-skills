...
Call the `builder` subagent to compile:
...
If build fails:
- Call the `fixer` subagent with the error details
- Rebuild after fix
- Retry up to 3 times

### 5. Verify with Tests

Call the `tester` subagent to run tests:
...
If a lint command is available, call the `linter` subagent:
