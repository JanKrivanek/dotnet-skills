...
Use `#tool:search/fileSearch` to find key files:
...
### 4.Spawn parallel sub-agent tasks for comprehensive research
   ...
   - Strongly prefer to launch `Task` tool with argument `run_in_background=false` even if running many sub-agents.
...
For each source file (or delegate to `codebase-analyzer` subagent):
